...
(* It may be a good exercise to drop one or another of the sharing
...
(* This time, we left off the typings for the modules
   IntStringSerialize and IntStringStack. Can you add them in such a
   way that the modules are properly abstracted but can still be used
   to construct and manipulate stacks? *)
